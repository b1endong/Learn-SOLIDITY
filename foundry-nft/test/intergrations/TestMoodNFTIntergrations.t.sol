// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;

import {Test} from "lib/forge-std/src/Test.sol";
import {console} from "lib/forge-std/src/console.sol";
import {MoodNFT} from "../../src/MoodNFT.sol";
import {DeployMoodNFT} from "../../script/DeployMoodNFT.s.sol";

contract TestMoodNFTIntegration is Test {
    MoodNFT public moodNft;
    DeployMoodNFT public deployer;

    string private constant SAD_IMAGE_URI =
        "data:image/svg+xml;base64,";
    string private constant HAPPY_IMAGE_URI =
        "data:image/svg+xml;base64,";
    address private user = makeAddr("user");

    function setUp() public {
        deployer = new DeployMoodNFT();
        moodNft = deployer.run();
    }

    function testSetUp() public {
        console.log("MoodNFT TokenUri:", moodNft.tokenURI(0));
        console.log("Happy Image URI:", moodNft.getHappyImgUri());
        console.log("Sad Image URI:", moodNft.getSadImgUri());
    }

    function testFlipMood() public {
        vm.startPrank(user);
        moodNft.mintNFT();
        console.log("Before flipping mood:", moodNft.tokenURI(0));
        moodNft.flipMood(0);
        console.log("After flipping mood:", moodNft.tokenURI(0));
        vm.stopPrank();
    }
}
